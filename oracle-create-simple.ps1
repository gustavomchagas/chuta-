# Script SIMPLIFICADO para criar instância Oracle Cloud
# Versão direta sem muitas verificações
...
try {
    ...
    Write-Host "Caminho esperado: $HOME\bin\oci.exe" -ForegroundColor Yellow
    exit 1
}
...
while ($attempt -lt $maxAttempts) {
    ...
    # Comando OCI CLI direto com timeout
    try {
        # Monta argumentos como array (evita problemas de escape)
        $args = @(
            ...
            '--ssh-authorized-keys-file', 'C:\Users\Gustavo\Documents\.ssh\oracle-chutai.pub'
        )
        
        $output = & oci @args 2>&1 | Out-String
        ...
        # Verifica erro
        if ($output -match "Out of capacity|OutOfCapacity|InternalError|500") {
        ...
        elseif ($output -match "LimitExceeded") {
            ...
            Write-Host "Verifique se ja tem instancias Always Free rodando." -ForegroundColor Yellow
            exit 1
        }
        elseif ($output -match "authorization failed|NotAuthenticated") {
            Write-Host ""
            Write-Host "ERRO: Falha de autenticacao!" -ForegroundColor Red
            Write-Host "Verifique se a API Key foi adicionada corretamente." -ForegroundColor Yellow
            exit 1
        }
        else {
            $shortError = $output.Substring(0, [Math]::Min(150, $output.Length))
